data "cloudinit_config" "controller" {
  ...
  part {
    ...
    content = templatefile("${path.module}/userdata/02-install-kinesis-agent.sh", {
      ingestion_stream = local.ingestion_stream
    })
  }
  ...
  part {
    ...
    content_type = "text/x-shellscript"
    content      = file("${path.module}/userdata/04-start-apache-fake-log-gen.sh")
  }

  part {
    filename     = "05-start-ssm-agent.sh"
    content_type = "text/x-shellscript"
    content      = file("${path.module}/userdata/05-start-ssm-agent.sh")
  }
}
